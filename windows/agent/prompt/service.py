...
from windows.agent.views import AgentData
from windows.desktop.views import DesktopState
...
class Prompt:
    ...
    def system_prompt(browser: str,language: str,tools_prompt:str,max_steps:int,instructions: list[str]=[]) -> str:
        ...
        template =PromptTemplate.from_file(files('windows.agent.prompt').joinpath('system.md'))
    ...
    def action_prompt(agent_data:AgentData) -> str:
        template = PromptTemplate.from_file(files('windows.agent.prompt').joinpath('action.md'))
        return template.format(**{
    ...
    def observation_prompt(query:str,steps:int,max_steps:int, tool_result:ToolResult,desktop_state: DesktopState) -> str:
        ...
        template = PromptTemplate.from_file(files('windows.agent.prompt').joinpath('observation.md'))
        return template.format(**{
            'steps': steps,
            'max_steps': max_steps,
    ...
    def answer_prompt(agent_data: AgentData, tool_result: ToolResult):
        template = PromptTemplate.from_file(files('windows.agent.prompt').joinpath('answer.md'))
        return template.format(**{
            'evaluate': agent_data.evaluate,
            'memory':  agent_data.memory,
            'thought': agent_data.thought,
